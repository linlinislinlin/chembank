---
type: learning-outcome
id: 11.5-C4
aliases:
- 11.5-C4
- LO 11.5-C4
parent_code: '11.5'
text: Describe the test to distinguish between saturated and unsaturated hydrocarbons
  by their reaction with aqueous bromine
tags:
- learning-outcome
- lo/11/5-C4
---

# LO 11.5-C4

**LO id:** `11.5-C4`

Describe the test to distinguish between saturated and unsaturated hydrocarbons by their reaction with aqueous bromine

Parent topic: [[syllabus/11.5|11.5]]

## Questions

```dataview
TABLE ms_answer, year, paper, question, syllabus_codes
FROM "questions"
WHERE contains(learning_outcomes, "11.5-C4")
SORT year ASC, paper ASC, question ASC
```

### Linked from last export

- [[cie-0620-2021-mj-p41-q7d-i]]
